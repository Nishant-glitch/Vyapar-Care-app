...
import { IEC_DISCLAIMER_TEXT } from '../../config/iecConfig';
import { COLORS } from '../../constants/theme';
import { useAuth } from '../../contexts/AuthContext';
import { useIECForm } from '../../contexts/IECFormContext';
import { formatINR } from '../../utils/currency';

export default function IECPaymentScreen({ navigation }) {
  const insets = useSafeAreaInsets();
  const { user } = useAuth();
  const { fees, submit, submitting } = useIECForm();
  ...
  return (
    <SafeAreaView style={styles.container} edges={['left', 'right']}>
      ...
      <ScreenHeader title="DGFT & Service Payment" />
      <IECStepper currentStep={10} />
      ...
      >
        ...
        <View style={styles.feeCard}>
          ...
          <Text style={styles.feeCardSub}>
            Import Export Code (IEC) Registration & DGFT Application Filing
          </Text>

          <View style={styles.feeRow}>
            <View style={{ flex: 1 }}>
              <Text style={styles.feeLabel}>DGFT Government Application Fee</Text>
              <Text style={styles.feeHint}>Official statutory fee paid on Bharatkosh / DGFT portal</Text>
            </View>
            <Text style={styles.feeVal}>{formatINR(fees.governmentFee)}</Text>
          </View>
          ...
          <View style={styles.feeRow}>
            <View style={{ flex: 1 }}>
              <Text style={styles.feeLabel}>Professional CA & Assisted-Filing Fee</Text>
              <Text style={styles.feeHint}>
                Document validation, PFMS bank check, Aadhaar/DSC e-sign & e-IEC delivery
              </Text>
            </View>
        ...
        <View style={styles.disclaimerBox}>
          ...
          <Text style={styles.disclaimerText}>{IEC_DISCLAIMER_TEXT}</Text>
        </View>
      </ScrollView>
