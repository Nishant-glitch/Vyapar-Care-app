...
export default function UploadDocumentsScreen({ navigation, route }) {
  ...
  const handlePress = async (doc) => {
    ...
    let file;
    try {
      const [picked] = await pick({ type: [types.images, types.pdf] });

      // Android ka content:// uri cache me local file bana lete hain — Expo ke
      // copyToCacheDirectory: true jaisa hi, taaki upload wala fetch(uri) chale.
      const [copy] = await keepLocalCopy({
        files: [{ uri: picked.uri, fileName: picked.name || doc.id }],
      ...
      // user ne picker band kar diya — chup-chaap wapas, status pending hi rahe
